...
router.post('/:id/compile-latex', asyncHandler(async (req, res) => {
  ...
  try {
    await new Promise((resolve, reject) => {
      exec('pdflatex --version', (error, stdout, stderr) => {
        if (error) {
          console.warn('pdflatex not available:', error.message);
          reject(new Error('LaTeX compiler not available on this system'));
        } else {
          resolve(stdout);
        }
      });
  ...
  const tmpDir = await new Promise((resolve, reject) => {
    fs.mkdtemp(path.join(os.tmpdir(), 'latex-'), (err, folder) => {
      if (err) reject(err);
      else resolve(folder);
    });
  });
  ...
  try {
    ...
    await fs.promises.writeFile(texFile, latexContent, 'utf8');

    // Compile LaTeX to PDF using pdflatex (run twice for references)
      ...
      try {
        await new Promise((resolve, reject) => {
          exec(`pdflatex -interaction=nonstopmode -output-directory="${tmpDir}" "${texFile}"`, (error, stdout, stderr) => {
            if (error) {
              console.warn(`pdflatex pass ${pass} error:`, stderr);
              compilationError = stderr || error.message;
              reject(error);
            } else {
              resolve(stdout);
            }
        ...
        compilationError = null; // Clear error if this pass succeeded
      } catch (passError) {
        // Continue to next pass even if this one failed
        console.warn(`pdflatex pass ${pass} failed, continuing to next pass`);
      }
    }

    // Check if PDF file exists (successful compilation)
    if (fs.existsSync(pdfFile)) {
      // Read compiled PDF and encode to base64
      const pdfData = await fs.promises.readFile(pdfFile);
      ...
      // PDF not generated, return error with LaTeX content
      await fs.promises.rm(tmpDir, { recursive: true, force: true });
      res.status(500).json({
        success: false,
        error: compilationError || 'PDF was not generated despite compilation attempts',
        data: {
          latexContent
        }
      });
    ...
    res.status(500).json({
      ...
      data: {
        latexContent // Return LaTeX content on failure so user can debug
      }
    });
  }
}));
...
// @route   POST /api/resumes/optimize-text
// @desc    Optimize resume text using Gemini API
// @access  Private
router.post("/optimize-text", asyncHandler(async (req, res) => {
  ...
  try {
    const optimizedData = await aiService.optimizeResumeText(resumeText, jobDescription);
    ...
    console.error("Resume text optimization failed:", error);
    res.status(500).json({ 
      success: false, 
...
// @route   POST /api/resumes/optimize-latex
// @desc    Optimize resume and generate LaTeX using AI API with fallback
// @access  Private
router.post("/optimize-latex", asyncHandler(async (req, res) => {
  const { resumeText, jobDescription } = req.body;
  ...
  try {
    const latexContent = await aiService.optimizeResumeAndGenerateLatex(resumeText, jobDescription);

    res.json({
      success: true,
      message: "Resume optimized and LaTeX generated successfully",
      data: {
        latex: latexContent
      }
    });
    ...
    if (error.message.includes('All AI services are currently unavailable')) {
      return res.status(503).json({
        success: false,
        error: "AI service is currently unavailable. Please check your OpenAI and Gemini API keys and billing status."
      });
    }
...
// @route   POST /api/resumes/compile-latex
// @desc    Compile LaTeX content to PDF without requiring resume ID
// @access  Public (no authentication required for standalone use)
router.post("/compile-latex", asyncHandler(async (req, res) => {
  const { latexContent } = req.body;
  ...
  try {
    ...
    if (fs.existsSync(pdfFile)) {
      ...
      res.status(500).json({
        success: false,
        error: 'PDF compilation failed: ' + (compilationError || 'Unknown error'),
        message: 'LaTeX compilation encountered errors. Please download the .tex file and compile it locally.'
      });
    }
  } catch (error) {
    // Cleanup temp directory on error
