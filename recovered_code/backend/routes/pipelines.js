...
router.post('/', asyncHandler(async (req, res) => {
  const { type, resumeId, jobDescription } = req.body;
  ...
  res.status(201).json({ success: true, data: { pipeline: { id: pipeline._id, type: pipeline.type, resume: pipeline.resume, jobDescription: pipeline.jobDescription, stages: pipeline.stages, metadata: pipeline.metadata } } });
}));
