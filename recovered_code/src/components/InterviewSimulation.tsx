...
const InterviewSimulation = ({
  ...
  const defaultQuestions = {
    ...
    Technical: [
      ...
      "Explain the four pillars of object-oriented programming: encapsulation, inheritance, polymorphism, and abstraction.",
      "How do you create objects in JavaScript/TypeScript?",
      "What is the difference between a class and an object?",
      "Describe inheritance and give an example of how it works with objects.",
    ],
  };

  const questions = fetchedQuestions ?? defaultQuestions;
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      try {
        ...
        if (qs)
          setFetchedQuestions(
            qs.map((q: any) => ({ question: q.question || q.text }))
          );
      } catch (err) {
  ...
  const startInterview = () => {
    ...
    setTimeElapsed(0);
    try {
      pipelineLib.updateStage("interview", true);
    } catch (e) {
      /* ignore */
    }
  };
  ...
  const endInterview = () => {
    ...
    // Navigate to results
    try {
      pipelineLib.updateStage("analytics", true);
    } catch (e) {
      /* ignore */
    }
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-yellow-700">
                            Enable both for the best interview experience and
                            accurate analysis.
                          </p>
                        </div>
                      </div>
            ...
            <Card className="bg-white/10 backdrop-blur-sm border-gray-600">
              ...
              <CardContent>
                <div className="space-y-4">
                  <Progress
                    value={
                      (currentQuestion / questions[interviewType].length) * 100
                    }
                    className="h-3"
                  />
                  <div className="text-white text-sm text-center">
                    {currentQuestion} / {questions[interviewType].length}{" "}
                    questions completed
                  </div>
                </div>
              </CardContent>
