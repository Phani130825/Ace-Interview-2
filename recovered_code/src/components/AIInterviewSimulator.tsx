    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (response.data.success && response.data.session) {
          ...
          setInterviewType(response.data.session.interviewType);
...
`;
      ...
      const type = typeLine
        ? typeLine.replace("Interview Type:", "").trim()
        : "General";

      const greeting = text.replace(typeLine, "").trim();

      const initialLog: LogEntry[] = [
        {
          speaker: "System" as const,
          text: `Starting interview simulation...`,
          timestamp: Date.now(),
        },
        {
      ...
      toast({
        ...
        description: `This is a ${type} interview.`,
      });
    } catch (error: any) {
...
5. Specific recommendations for improvement`;
        ...
        toast({
          ...
          description: "Your interview session has concluded.",
        });
      } else {
        // Generate next question
        const nextQuestionPrompt = `Continue this ${interviewType} interview for the ${
          jobDetails.role
        } position at ${
          jobDetails.company
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <p className="text-center text-muted-foreground">
          Enter your details to begin your AI-powered interview simulation
        </p>
      </CardHeader>
  ...
  const renderFeedback = () => (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <p className="text-center text-muted-foreground">
          Your interview session has concluded. Review the conversation below.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
