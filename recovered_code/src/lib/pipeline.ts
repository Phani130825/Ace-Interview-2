...
export function createPipeline(type: PipelineType, resumeId?: string, jobDescription?: Pipeline['jobDescription']) {
  ...
  try {
    ...
    if (token) {
      fetch((import.meta.env.VITE_API_URL || '') + '/pipelines', {
        ...
        body: JSON.stringify({ type, resumeId, jobDescription })
      }).catch(() => { /* ignore */ });
    }
  } catch (e) { /* ignore */ }
